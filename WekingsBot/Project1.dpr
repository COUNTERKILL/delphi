...
{$R *.res}
const
  ERROR_REG_TEXT_START = 'Для регистрации сообщите автору код ';
  ERROR_REG_TEXT_END = #13#10'Cтоимость регистрации 250 рублей'+#13#10+'Подробнее на http://wbot.xedus.ru/';
  ERROR_REG_CAPTION = 'Версия не зарегистрирована!';
  VERSION = '0.3.3';
var
  ID,Otvet:string;
  http:TIdHTTP;
...
begin
  ...
  if (Otvet<>'') and (VERSION<>Copy(Otvet,1,Pos(#13#10,Otvet)-1)) then
    MessageBox(Application.Handle,PWideChar('Вышла новая версия прграммы! '+#13#10+'Текущая версия: '+Copy(Otvet,1,Pos(#13#10,Otvet)-1)+#13#10+'Ваша версия: '+VERSION+#13#10+'Пожалуйста, обновите программу! http://wbot.xedus.ru/'+#13#10+#13#10+Copy(Otvet,Pos(#13#10,Otvet)+2,Length(Otvet))),PWideChar('Обновите программу!'),MB_ICONWARNING or MB_TOPMOST);
  try
    Otvet:=http.Get('http://xedus.ucoz.ru/bot.txt');
  except
    MessageBox(Application.Handle,PWideChar('Проверьте подключение к интернету и попробуйте еще раз!'),PWideChar('Ошибка соединения!'),MB_ICONSTOP or MB_TOPMOST);
